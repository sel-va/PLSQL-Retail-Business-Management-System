...
CREATE OR REPLACE PROCEDURE SP_PRD_ODR_CUS_LST (PRD_ID IN NUMBER, CUS_DTLS_REFC OUT SYS_REFCURSOR)
AS
BEGIN
...
UNIT_TEST
	...
	EXEC SP_PRD_ODR_CUS_LST(1,:X);
	PRINT X
*/
-----------------------------------------------------------------------------------------------------------------------------
--NUMBER OF ORDER FOR PARTICULAR PRODUCT IN N DAYS

CREATE OR REPLACE PROCEDURE SP_NO_ODR_FR_PRD (PRD_ID IN NUMBER, DAYS_TO_CNT IN NUMBER, NUM_ORD_REFC OUT SYS_REFCURSOR)
AS
BEGIN
...
CREATE OR REPLACE PROCEDURE SP_NOT_SALE_PRD (DAYS_TO_CNT IN NUMBER, NOT_SALE_REFC OUT SYS_REFCURSOR)
AS
BEGIN
...
--UNIT TEST
-- VARIABLE X REFCURSOR
--EXEC SP_GEN_REP_PRD_LESS_STK(STK_QTY=>30, LES_STK_REFC=>:X);
--PRINT X
